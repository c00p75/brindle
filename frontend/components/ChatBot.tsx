...
function BotCard({ bot }: { bot: any }) {
  return (
    <div style={{
      ...
      padding: 12, marginTop: 8, display: "flex", flexDirection: "column", gap: 4,
      ...
      {suggestions.map((s, i) => (
        <button
          ...
          style={{
            ...
            borderRadius: 20, fontSize: 12, color: "#6366f1", cursor: "pointer",
...
async function sendMessage(message: string, sessionId: string | null): Promise<ChatResponse> {
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6 }}>
      {actions.map((action, i) => (
        <span key={i} style={{
          ...
          background: "#e0e7ff", color: "#3730a3", borderRadius: 12, padding: "2px 8px",
        }}>
...
function TimeStamp({ ts }: { ts?: number }) {
  ...
  const d = new Date(ts);
  return (
    <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 4, paddingInline: 4 }}>
...
export default function ChatBot() {
  ...
  useEffect(() => { setAuthed(!!getToken()); }, [open]);
  ...
  const loadSessions = async () => {
    try { setSessions(await api.listChatSessions() as any); }
    ...
    <>
      <style>{`
        ...
        .chat-msg code {
          ...
          background: rgba(0,0,0,0.06); padding: 2px 4px; border-radius: 4px;
        }
        ...
        .chat-msg table { border-collapse: collapse; width: 100%; margin: 8px 0; font-size: 12px; }
        .chat-msg th, .chat-msg td { border: 1px solid #e2e8f0; padding: 6px 8px; text-align: left; }
        .chat-msg th { background: #f8fafc; font-weight: 600; }
        .chat-bubble { animation: fadeIn 0.25s ease; }
        .chat-suggestion:hover { background: #f0f0ff !important; border-color: #6366f1 !important; transform: translateY(-1px); }
        .chat-history-item:hover { background: #f1f5f9 !important; }
        .chat-textarea { resize: none; scrollbar-width: thin; }
        .chat-textarea:focus { border-color: #6366f1 !important; outline: none; }
      `}</style>
      ...
      <button
        ...
        style={{
          ...
          background: open ? "#4f46e5" : "#6366f1", border: "none", cursor: "pointer",
      ...
      {open && (
        <div style={{
          ...
          animation: "slideUp 0.2s ease", border: "1px solid #e0e7ff",
        }}>
          {/* Header */}
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 11, opacity: 0.8, color: "#fff" }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              {[
                { icon: isHistoryOpen ? "💬" : "🕘", title: isHistoryOpen ? "Back to chat" : "History", onClick: () => setIsHistoryOpen(!isHistoryOpen) },
                { icon: "＋", title: "New chat", onClick: startNewChat },
              ].map((btn, i) => (
                <button key={i} onClick={btn.onClick} title={btn.title} style={{
                  background: "rgba(255,255,255,0.15)", border: "none", color: "#fff",
                  padding: "6px 8px", borderRadius: 8, cursor: "pointer", fontSize: 14,
                  display: "flex", alignItems: "center", justifyContent: "center",
                }}>{btn.icon}</button>
              ))}
            </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {sessions.map((s) => (
                    <div
                      ...
                      style={{
                        padding: "12px 16px",
                        background: sessionId === s.id ? "#eef2ff" : "#fff",
                        border: `1px solid ${sessionId === s.id ? "#c7d2fe" : "#e2e8f0"}`,
                    ...
                    >
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <span style={{ fontSize: 13, fontWeight: 500, color: "#334155", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>
                          {s.title}
                        </span>
                        <span style={{ fontSize: 10, color: "#94a3b8" }}>
                          {new Date(s.updated_at_ms).toLocaleDateString()}
                        </span>
                      </div>
                      <button
                        onClick={(e) => deleteSession(e, s.id)}
                        style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", padding: 4, fontSize: 14 }}
                      >✕</button>
                    </div>
                  ))}
                </div>
            ...
            <div style={{
              ...
              {messages.length === 0 && (
                <div style={{ textAlign: "center", color: "#94a3b8", marginTop: 80 }}>
                  ...
                  <p style={{ fontSize: 12, marginTop: 8, color: "#64748b", maxWidth: 280, margin: "8px auto 0" }}>
                    I can manage bots, analyze performance, run backtests, and fetch live market data.
                  </p>
                  <SmartSuggestions onSelect={(s) => handleSend(s)} />
                </div>
              )}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      background: msg.isError ? "#fef2f2" : msg.role === "user" ? "#6366f1" : "#fff",
                      color: msg.isError ? "#991b1b" : msg.role === "user" ? "#fff" : "#1e293b",
                      fontSize: 13.5,
                      border: msg.isError ? "1px solid #fecaca" : msg.role === "assistant" ? "1px solid #e2e8f0" : "none",
                  ...
                  >
                    <ReactMarkdown
                      ...
                      components={{
                        a: ({ node, ...props }) => (
                          <a {...props} style={{ color: msg.role === "user" ? "#fff" : "#6366f1", textDecoration: "underline" }} target="_blank" rel="noopener noreferrer" />
                        ),
                      }}
                    >
                      {msg.content}
                  ...
                  {msg.entities && msg.entities.length > 0 && (
                    <div style={{ maxWidth: "85%", display: "flex", flexDirection: "column", gap: 4 }}>
                      {msg.entities.map((ent, j) => <BotCard key={j} bot={ent} />)}
                    </div>
                  )}

                  {/* Action pills */}
                  {msg.actions && msg.actions.length > 0 && (
                    <ActionPills actions={msg.actions} />
                  )}
                  ...
                  {msg.isError && (
                    <button
                      ...
                      style={{
                        ...
                        color: "#991b1b", cursor: "pointer", display: "flex",
          ...
          {/* Input bar */}
          <div style={{
            padding: "10px 12px", borderTop: "1px solid #e0e7ff", background: "#fff",
            ...
            <textarea
              ...
              style={{
                flex: 1, border: "1.5px solid #e0e7ff", borderRadius: 10,
                padding: "8px 12px", fontSize: 13.5, background: "#f8f9ff",
